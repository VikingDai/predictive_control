...
public:
   ...
   */
  void calculateJacobianMatrix(const Eigen::VectorXd& joints_angle, Eigen::MatrixXd& FK_Matrix,
                               Eigen::MatrixXd& Jacobian_Matrix);

  /**
   * @brief calculateForwardKinematicsUsingKDLSolver: Calculate forward kinematics start from root frame to tip link of
   * manipulator
   *                                                  by using kdl recursive solver
   * @param joints_angle: Current joint angle
   * @param FK_Matrix: Resultant Forward Kinematic Matrix
   */
  void calculateForwardKinematicsUsingKDLSolver(const Eigen::VectorXd& joints_angle, Eigen::MatrixXd& FK_Matrix);
   ...
   * @param Jacobian_Matrix: Resultant Jacobian Matrix
   */
  void calculateJacobianMatrixUsingKDLSolver(const Eigen::VectorXd& joints_angle, Eigen::MatrixXd& FK_Matrix,
                                             Eigen::MatrixXd& Jacobian_Matrix);
   ...
   */
  void calculateInverseJacobianbySVD(const Eigen::MatrixXd& jacobian, Eigen::MatrixXd& jacobianInv);
   ...
   * @param jacobianInv: Inverse of Jacobian Matrix
   */
  void calculateInverseJacobianbyDirect(const Eigen::MatrixXd& jacobian, Eigen::MatrixXd& jacobianInv);
   ...
   */
  void getGripperPoseVectorFromFK(const Eigen::MatrixXd& FK_Matrix, Eigen::VectorXd& vector);
...
private:
  ...
  /**
   * @brief generateTransformationMatrixFromJointValues: Generate transformation matrix used for computing forward
   * kinematics,
   ...
   */
  void generateTransformationMatrixFromJointValues(const unsigned int& current_segment_id, const double& joint_value,
