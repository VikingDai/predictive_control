...
bool predictive_control_ros::initialize()
{
  ...
  if (ros::ok())
  {
    ...
    if (pd_config_success == false || kinematic_success == false || collision_avoidance_success == false ||
        collision_success == false || static_collision_success == false || pd_traj_success == false ||
        pd_config_->initialize_success_ == false)
    {
      ...
      std::cout << "States: \n"
                ...
                << " pd config init success: " << std::boolalpha << pd_config_->initialize_success_ << std::endl;
      return false;
    }
    ...
    move_action_server_.reset(
        new actionlib::SimpleActionServer<predictive_control::moveAction>(nh, MOVE_ACTION_NAME, false));
...
void predictive_control_ros::runNode(const ros::TimerEvent& event)
{
  ...
  pd_trajectory_generator_->solveOptimalControlProblem(
      Jacobian_Matrix_, current_gripper_pose_, goal_gripper_pose_, collision_avoidance_->getDistanceCostFunction(),
      static_collision_avoidance_->collision_cost_vector_, controlled_velocity_);
...
void predictive_control_ros::moveGoalCB()
{
  if (move_action_server_->isNewGoalAvailable())
  {
    ...
    tracking_ = false;
    collision_detect_->createStaticFrame(move_action_goal_ptr->target_endeffector_pose,
                                         move_action_goal_ptr->target_frame_id);
...
int predictive_control_ros::moveCallBack(const predictive_control::moveGoalConstPtr& move_action_goal_ptr)
{
  ...
  if (move_action_goal_ptr->target_frame_id.empty() ||
move_action_goal_ptr->target_endeffector_pose.header.frame_id.empty())
  {
    tracking_ = true;
    result.reach = false;
    move_action_server_->setAborted(result, " Not set desired goal and frame id, Now use to intractive marker to set
desired goal");
    //target_frame_ = pd_config_->target_frame_;
  }
...
void predictive_control_ros::jointStateCallBack(const sensor_msgs::JointState::ConstPtr& msg)
{
  ...
  else
  {
    ...
    // calculate forward kinematic and Jacobian matrix using current joint values, get current gripper pose using
    // FK_Matrix
    ...
    collision_detect_->updateCollisionVolume(kinematic_solver_->FK_Homogenous_Matrix_,
                                             kinematic_solver_->Transformation_Matrix_);
...
void predictive_control_ros_node::run_node(const ros::TimerEvent& event)
{
  ...
  kinematic_solver_->compute_and_get_each_joint_pose(current_position_vec_copy, tranformation_matrix_stamped,
link_length);
...
std::endl;
  ...
  pd_frame_tracker_->optimal_control_solver(Jacobian_Mat, current_gripper_pose, target_gripper_pose,
joint_velocity_data); //, collision_distance_vector
  ...
  std::cout<<"\033[36;1m" << "***********************"<< "cartesian distance: " << cartesian_dist <<
"***********************" << std::endl
              << "***********************"<< "rotation distance: " << rotation_dist << "***********************" <<
std::endl
      << "\033[0m\n" << std::endl;
...
void predictive_control_ros::publishErrorPose(const Eigen::VectorXd& error)
{
  ...
  if (activate_output_)
  {
    std::cout << "\033[94m"
              << "publishErrorPose:"
              << " qx:" << stamped.pose.orientation.x << "qy:" << stamped.pose.orientation.y
              << "qz:" << stamped.pose.orientation.z << "qw:" << stamped.pose.orientation.w << "\033[0m" << std::endl;
  }
  // publish
...
bool predictive_control_ros::transformEigenToGeometryPose(const Eigen::VectorXd& eigen_vector,
                                                          geometry_msgs::Pose& pose)
{
  // traslation
  ...
  if (activate_output_)
  {
    std::cout << "\033[32m"
              << "publishErrorPose:"
              << " roll:" << eigen_vector(3) << " pitch:" << eigen_vector(4) << " yaw:" << eigen_vector(5) << "\033[0m"
              << std::endl;
  }
  ...
  if (tf_listener_.frameExists(to) && tf_listener_.frameExists(from))
  {
    try
    {
      ...
      if (activate_output_)
      {
        std::cout << "\033[94m"
                  << "getTransform:"
                  << " qx:" << stamped_tf.getRotation().getX() << "qy:" << stamped_tf.getRotation().getY()
                  << "qz:" << stamped_tf.getRotation().getZ() << "qw:" << stamped_tf.getRotation().getW() << "\033[0m"
                  << std::endl;
      }
      ...
      if (activate_output_)
      {
        std::cout << "\033[32m"
                  << "getTransform:"
                  << " roll:" << stamped_pose(3) << " pitch:" << stamped_pose(4) << " yaw:" << stamped_pose(5)
                  << "\033[0m" << std::endl;
      }
...
/*
bool predictive_control_ros::getTransform(const std::string& from, const std::string& to, geometry_msgs::PoseStamped&
stamped_pose)
{
  bool transform = false;
  tf::StampedTransform stamped_tf;
...
bool predictive_control_ros::checkPositionLimitViolation(const Eigen::VectorXd& joint_position,
  ...
  for (int i = 0u; i < size; ++i)
  {
    ...
    if ((min_position_limit_(i) - position_tolerance) >= joint_position(i))
    {
      ROS_WARN("%s lower position tolerance violate with current position %f required position %f",
               pd_config_->joints_name_.at(i).c_str(), joint_position(i),
               (min_position_limit_(i) - position_tolerance));
      return true;
    }
    ...
    else if ((max_position_limit_(i) + position_tolerance) <= joint_position(i))
    {
      ROS_WARN("%s upper position tolerance violate with current position %f required position %f",
               pd_config_->joints_name_.at(i).c_str(), joint_position(i),
               (max_position_limit_(i) + position_tolerance));
      return true;
    }

    // Current position is within range of minimum and maximum position limit
    else if (activate_output_ && (((min_position_limit_(i) + position_tolerance) - joint_position(i)) <= 0.0) &&
...
// check position lower and upper limit violation
bool predictive_control_ros::checkVelocityLimitViolation(const std_msgs::Float64MultiArray& joint_velocity,
  ...
  for (int i = 0u; i < size; ++i)
  {
    ...
    if ((min_velocity_limit_(i) - velocity_tolerance) >= joint_velocity.data[i])
    {
      ROS_WARN("%s lower velocity tolerance violate with current velocity %f required velocity %f",
               pd_config_->joints_name_.at(i).c_str(), joint_velocity.data[i],
               (min_velocity_limit_(i) - velocity_tolerance));
      return true;
    }
    ...
    else if ((max_velocity_limit_(i) + velocity_tolerance) <= joint_velocity.data[i])
    {
      ROS_WARN("%s upper velocity tolerance violate with current velocity %f required velocity %f",
               pd_config_->joints_name_.at(i).c_str(), joint_velocity.data[i],
               (max_velocity_limit_(i) + velocity_tolerance));
      return true;
    }

    // Current velocity is within range of minimum and maximum velocity limit
    else if (activate_output_ && (((min_velocity_limit_(i) + velocity_tolerance) - joint_velocity.data[i]) <= 0.0) &&
                                                     ...
                                                     const double& position_tolerance)
{
  ...
  for (int i = 0u; i < size; ++i)
  {
    ...
    // Current position is above than maximum position limit + tolerance, check upper limit violation
             ...
             (std::abs(joint_position(i) - min_position_limit_(i)) >
              std::abs(max_position_limit_(i) - joint_position(i))))
    {
      ROS_WARN("%s upper position tolerance violate with current position %f required position %f",
               pd_config_->joints_name_.at(i).c_str(), joint_position(i),
               (max_position_limit_(i) + position_tolerance));
                                                     ...
                                                     const double& velocity_tolerance)
{
  ...
  for (int i = 0u; i < size; ++i)
  {
    ...
    if (((min_velocity_limit_(i) - velocity_tolerance) >= joint_velocity.data[i]) &&
        (std::abs(joint_velocity.data[i] - min_velocity_limit_(i)) <
         std::abs(max_velocity_limit_(i) - joint_velocity.data[i])))
    {
